...
def init_database():
    ...
    # Beispieldaten
    students = [
        {'name': 'Freda Kids', 'level': 'HF', 'age': 20},
        {'name': 'Sam Sung', 'level': 'HF', 'age': 22},
        {'name': 'Chris P. Bacon', 'level': 'AP', 'age': 21},
        {'name': 'Willie Makit', 'level': 'ICT', 'age': 23},
        {'name': 'Saad Maan', 'level': 'PE', 'age': 24}
    ]
...
@app.post('/exampledata')
def create_exampledata():
    init_database()
    return {'message': 'Example data created'}


@app.delete('/cleardatabase')
def delete_database():
    db.drop_all()
    db.create_all()
